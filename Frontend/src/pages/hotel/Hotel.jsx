...
const Hotel = (props) => {
  ...
  const verifyPayment = async (paymentId) => {
    try {
      console.log("verify ", paymentId);
      console.log("verify ", roomData);
      await axios.post("http://localhost:5000/api/payment/verifyPayment", {
        paymentId,
        amount: total,
        orderId,
        rooms: selectedRooms,
  ...
  const getHotel = () => {
    axios.get(`http://localhost:5000/api/room/hotel/${id}`).then((res) => {
      setHotel(res.data);
    });
  };
